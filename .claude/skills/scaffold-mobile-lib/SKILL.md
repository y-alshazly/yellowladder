...
- **React Native Paper** for UI — never custom primitives. Paper theme is **MD3 + LTR only**.
...
- **`react-i18next`** for translations. Yellow Ladder supports `en` (default), `de`, `fr` — update all three catalogs (`en.json`, `de.json`, `fr.json`) in the same commit. **LTR only** — do NOT call `I18nManager.forceRTL` or branch on `I18nManager.isRTL`.
- **RBAC UI gating** via `<HasPermission permission={Permissions.XxxYyy}>` or the `useHasPermission(Permissions.XxxYyy)` hook from `@yellowladder/shared-mobile-ui`. Import `Permissions` from `@yellowladder/shared-types`.
