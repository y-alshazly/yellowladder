...
| `i18n`      | `type:i18n`        | `platform:any`    | react-i18next config, `en.json` / `de.json` / `fr.json` (EN default, DE + FR), ICU plurals |
| `web-ui`    | `type:web-ui`      | `platform:web`    | MUI theme (LTR only), composite components                                                 |
| `mobile-ui` | `type:mobile-ui`   | `platform:mobile` | Paper theme (MD3 + LTR only), PaperProvider, composite components                          |
